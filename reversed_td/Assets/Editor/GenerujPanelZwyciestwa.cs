...
public static class GenerujPanelZwyciestwa
{
    [MenuItem("Tools/UI/Generuj Panel Zwyciestwa")]
    public static void Generate()
    {
        Canvas canvas = Object.FindFirstObjectByType<Canvas>();
        if (canvas == null) { Debug.LogError("[GenerujPanelZwyciestwa] Brak Canvas w scenie!"); return; }
        ...
        Color accent = new Color(0.25f, 1f, 0.40f);
        Color bg     = new Color(0f, 0.06f, 0f, 0.90f);

        var panel = BuildPanel(canvas.transform, "VictoryPanel", "ZWYCIESTWO!", accent, bg,
            hasRestart: true,
            out var statsText,
            out var restartBtn,
            out var menuBtn,
        ...
        Undo.RegisterCreatedObjectUndo(panel, "Generuj Panel Zwyciestwa");

        if (gm != null)
        {
            gm.victoryPanel         = panel;
            gm.victoryStatsText     = statsText;
            gm.victoryRestartButton = restartBtn;
            gm.victoryMenuButton    = menuBtn;
            gm.victoryLobbyButton   = lobbyBtn;
            ...
            Debug.Log("[GenerujPanelZwyciestwa] Podlaczono do GameManager.");
        }
        else
        {
            Debug.LogWarning("[GenerujPanelZwyciestwa] Brak GameManager — podlacz recznie:\n" +
                "  victoryPanel        → VictoryPanel\n" +
                "  victoryStatsText    → VictoryPanel/StatsText\n" +
                "  victoryRestartButton→ VictoryPanel/Buttons/BtnRestart\n" +
                "  victoryMenuButton   → VictoryPanel/Buttons/BtnMenu\n" +
                "  victoryLobbyButton  → VictoryPanel/Buttons/BtnLobby");
        }

        EditorSceneManager.MarkSceneDirty(EditorSceneManager.GetActiveScene());
        Selection.activeGameObject = panel;
        Debug.Log("[GenerujPanelZwyciestwa] Panel wygenerowany.");
    }

    static GameObject BuildPanel(Transform parent, string name, string title,
        Color accent, Color bg, bool hasRestart,
        ...
        out Button restartOut, out Button menuOut, out Button lobbyOut)
    {
        ...
        restartOut = hasRestart
            ? Btn(btnBar.transform, "BtnRestart", "Zagraj ponownie", new Color(0.20f, 0.55f, 0.80f))
            : null;
